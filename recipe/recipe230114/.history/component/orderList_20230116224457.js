export default function Orderlist({ $target }) {
  ...
  let orderListArray = '';
for (let orderListArray = 0; orderListArray < array.length; orderListArray++) {
  const element = array[orderListArray];
}
            ...
            <h3>
              ...
              ${this.state.data.map(item => `
                <div class="order__menu">
                  <p class="order__menu__name">${item.orderList[element].name} <span>${item.orderList[element].quantity}</span></p>
                  <ul class="order__menu__option">
                  <li>
                  ▶ ${item.orderList[element].options} 
                  <!-- <span>${item.orderList[element].options} </span> -->
                </li>
                <li>▶ ${item.orderList[element].options}  <span>1</span></li>
                <li>▶ ${item.orderList[element].options}  <span>1</span></li>
              </ul>
