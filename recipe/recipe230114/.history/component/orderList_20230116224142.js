export default function Orderlist({ $target }) {
  ...
  let orderListArray = 0
  for (let orderListArray = 0; orderListArray < 10;) {
            ...
            <h3>
              ...
              ${this.state.data.map(item => `
                <div class="order__menu">
                  <p class="order__menu__name">${item.orderList[orderListArray].name} <span>${item.orderList[orderListArray].quantity}</span></p>
                  <ul class="order__menu__option">
                  <li>
                  ▶ ${item.orderList[orderListArray].options} 
                  <!-- <span>${item.orderList[orderListArray].options} </span> -->
                </li>
                <li>▶ ${item.orderList[orderListArray].options}  <span>1</span></li>
                <li>▶ ${item.orderList[orderListArray].options}  <span>1</span></li>
              </ul>
