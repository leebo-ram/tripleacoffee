export default function Orderlist({ $target }) {
  ...
  this.render = () => {
      ...
      if (this.state.data != '') {
          ...
          <h3>주문목록 <span class="orderList__count">총 ${item.order.orderList.quantity}건</span>
            <h3>
              <div class="orderList">
              ...
              ${this.state.data.map(
                ...
                <div class="order__menu">
                  <p class="order__menu__name">${item.order.orderList[0].options[0].optionName} <span>${item.order.orderList.options[0].optionQuantity}</span></p>
                  <ul class="order__menu__option">
                    <li>
                      ▶ ${item.order.orderList[0].options[0].optionName} 
                      <!-- <span>1개</span> -->
                    </li>
                    <li>▶ ${item.order.orderList.options[0].optionQuantity}<span>1개</span></li>
