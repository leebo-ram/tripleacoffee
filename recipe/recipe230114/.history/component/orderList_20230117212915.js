export default function Orderlist({ $target }) {
  ...
  this.$element.className = 'recipe_orderList'; //recipe1만 복붙 
            ...
            <h3>
              ...
              ${this.state.data[0].orderList.map(item => `
                <div class="order__menu">
                  <p class="order__menu__name">${item.name} <span>${item.quantity}</span></p>
                  <ul class="order__menu__option">
                  <li>
                  ▶ ${item.options} 
                  <!-- <span>${item.options} </span> -->
                </li>
                <li>▶ ${item.options}  <span>1</span></li>
                <li>▶ ${item.options}  <span>1</span></li>
              </ul>
