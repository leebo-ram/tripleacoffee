...
window.addEventListener('click', (e) => {
    ...
    console.log(e.target);
    if (e.target.closest('button')) {
        if (e.target.closest('button').classList.contains('order__menu__btn')) {
            const active = document.querySelector('.order__menu__btn.selected');
            if (active != null) {
                active.classList.remove('selected');
            }
            if (!e.target.closest('button').classList.contains('selected')) {
