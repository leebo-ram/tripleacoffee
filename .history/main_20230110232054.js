...
window.addEventListener('click', (e) => {
    ...
    if (e.target.closest('div')) {
        ...
        // 해제 btn
        function topNode(parentNode) {
            const recipeTopNode = document.querySelector('.order__menu');
        }

        if (e.target.closest('div').classList.contains('order__menuBtns')) {
            const active = document.querySelector('.order__menuBtns.selected');
            const fontColor = topNode(
                document.querySelector('.order__menu.decoration')
            );
            if (active != null) {
                active.classList.remove('selected');
                if (
                    e.target
                        .closest('div')
                        .classList.contains('order__menuBtns')
                ) {
                    if (fontColor != null) {
                        fontColor.classList.remove('decoration');
                    }
                }
            }
            if (!e.target.closest('div').classList.contains('selected')) {
                e.target.closest('div').classList.add('selected');
                if (!e.target.closest('p').classList.contains('decoration')) {
                    e.target.closest('p').classList.add('decoration');
                }
            }
            return;
        }
