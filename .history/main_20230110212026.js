...
window.addEventListener('click', (e) => {
    ...
    if (e.target.closest('button')) {
        if (
            e.target
            .closest('button')
            .classList.contains('oder__menu__complete')
            console(e.target.closest);
        ) {
            const active = document.querySelector(
                '.oder__menu__complete.finish'
            );
            if (active != null) {
                active.classList.remove('finish');
            }
            if (!e.target.closest('button').classList.contains('finish')) {
                e.target.closest('button').classList.add('finish');
            }
            return;
        }
    }
