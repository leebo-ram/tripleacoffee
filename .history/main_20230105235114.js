...
window.addEventListener('click', (e) => {
  ...
  const target2 = document.getElementById('certification__Num');
  if(e.target2.closest('button').dataset.val) {
    if(target2.value.length < 11) {
      target2.value += e.target.closest('button').dataset.val;
      console.log(target2.value.length) 
    }
    
  }
  if(e.target2.closest('button').className == 'number__del__ALL' ) {
    target.value = "";
  }
  if(e.target2.closest('button').className == 'number__pop') {
